...
public class Transformacao2 {
    ArrayList<String> PasadenaStations = new ArrayList<>();

    public Transformacao2(){
        iniciaArrrayList();
    }

    public void iniciaArrrayList(){
        try {
            String path = "./arquivos/pivot_stations.csv";
            BufferedReader br = new BufferedReader(new FileReader(path));
            br.readLine();
            String line;
            while((line = br.readLine()) != null){
                PasadenaStations.add(line);
            }
            br.close();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void filtraArquivo(){
        ...
        try {
            ...
            while((line = br.readLine()) != null){
                ...
                if(this.PasadenaStations.contains(linha[9]) || this.PasadenaStations.contains(linha[10])){
        ...
        try {
            File product = new File("./arquivos/LAMetroTrips_F1.csv");//determina onde será escrito
